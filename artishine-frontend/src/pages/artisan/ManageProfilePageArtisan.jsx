...
import { Plus, Edit3, Trash2, Undo2 } from 'lucide-react';
...
const ManageProductsPage = () => {
  ...
  // Undo-delete
  const [lastDeleted, setLastDeleted] = useState(null); // {product, index}
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      ...
      // Store for undo
      const idx = products.findIndex(p => (p.product_id || p.id || p.productId) === productId);
      const deletedProduct = products[idx];
      setLastDeleted({ product: deletedProduct, index: idx });

      // Remove from UI
      setProducts(prev => prev.filter(p => (p.product_id || p.id || p.productId) !== productId));

      // Close modal if open
      if (selectedProduct && (selectedProduct.product_id || selectedProduct.id) === productId) {
      ...
      toast.success(
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="underline text-white hover:text-amber-200"
          >
            Undo
          </button>
        </div>,
  ...
  const handleSaveEdit = async () => {
    ...
    const productId = editingProduct.product_id || editingProduct.id || editingProduct.productId;
    const updates = {
  ...
  const formatDateToIST = (isoString) => {
    if (!isoString) return '—';
    try {
      return new Date(isoString).toLocaleString('en-GB', { timeZone: 'Asia/Kolkata' });
    } catch {
      return isoString;
    }
  };
      ...
      <div className="p-6 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <PrimaryButton onClick={() => navigate('/upload')} icon={<Plus className="h-5 w-5" />}>
              Add Product
            </PrimaryButton>
          </div>
          ...
          {products.length === 0 ? (
            <div className="text-center py-20 bg-amber-50/50 rounded-2xl p-12">
              ...
              <h2 className="text-2xl font-serif font-bold mb-2 text-amber-900">No products yet</h2>
              <p className="text-amber-700 mb-6">Create your first masterpiece and share it with the world</p>
              <PrimaryButton onClick={() => navigate('/upload')} size="lg" icon={<Plus className="h-5 w-5" />}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {products.map((product) => {
                ...
                return (
                  <article key={pid} className="group">
                    <div
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <span className="text-xs text-amber-600">{product.story?.Category || '—'}</span>
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-2xl font-serif font-bold text-amber-900">
                  {editingProduct ? 'Edit Product' : (selectedProduct.story?.Title || 'Untitled')}
                </h2>
                {editingProduct && (
                  <p className="text-sm text-amber-700 mt-1">Make changes and click Save</p>
                )}
              </div>
                ...
                {editingProduct ? (
                  ...
                  <>
                    ...
                    <PrimaryButton
                      variant="destructive"
                      size="sm"
                      onClick={() => handleDelete(selectedProduct.product_id || selectedProduct.id)}
                    ...
                    >
                      {deletingId === (selectedProduct.product_id || selectedProduct.id) ? 'Deleting...' : 'Delete'}
                    </PrimaryButton>
                  </>
                )}
                <button onClick={closeProduct} className="ml-2 text-amber-700 underline text-sm">
                  Close
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={key} className="bg-white/60 p-4 rounded-lg border border-amber-100">
                    <p className="text-xs font-medium text-amber-700 uppercase tracking-wider mb-1">{label}</p>
                    {editingProduct ? (
                      <input
                        type="text"
                      ...
                      <p className="text-amber-900">{selectedProduct.story?.[key] || '—'}</p>
                    )}
                  </div>
                ))}
                ...
                {/* Timestamp */}
                <div className="bg-white/60 p-4 rounded-lg border border-amber-100">
                  <p className="text-xs font-medium text-amber-700 uppercase tracking-wider mb-1">Created</p>
